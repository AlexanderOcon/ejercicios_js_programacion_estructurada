...
// 4 Ordenar las mascotas alfabéticamente y si tienen el mismo tipo, por  ascendente usando sort.
mascotas.sort((a, b) => (a.nombre.localeCompare(b.nombre)));
mascotas.sort((a, b) => (a.tipo.localeCompare(b.tipo)));
console.log(mascotas);
...
let nombresMayus = mascotas.map(mascotas => mascotas.nombre.toUpperCase());
console.log(nombresMayus);

// 7 Calcular el peso total de todas las mascotas usando reduce.
let pesoTotal = mascotas.reduce((total, mascota) => total + mascota.peso, 0);
console.log(pesoTotal); // 117.8kg
